...
def main():
    ...
    for cl in myList:
        imgCur = cv2.imread(os.path.join(path, cl))
    ...
    #prompt user to input the image name they want to detect
    img_name = input("Enter the image name you want to detect (e.g., 'blue 0, red chance cut, etc'): ")
    img_path = os.path.join(path, img_name + '.png')

    #reading image name from path
